...
export default function Curriculum() {
  ...
  const [snapStrict, setSnapStrict] = useState(true) // lock to one year at a time
    ...
    <section id="curriculum" className="py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 text-center">
          ...
          <p className="mt-4 text-lg text-gray-700">Scroll through the 4-year journey. Each year features focused modules you will study.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-[220px,1fr] gap-6">
          {/* Sticky mini-year sidebar (desktop) */}
          <div className="hidden md:block">
            <div className="sticky top-24 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-xs font-semibold tracking-widest text-blue-600">YEARS</span>
                <button
                  onClick={() => setSnapStrict((s) => !s)}
                  className="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-medium text-gray-800 transition-colors hover:border-blue-300"
                >
                  ...
                  <li key={y.year}>
                    <button
                      ...
                      className={`group w-full rounded-xl px-3 py-2 text-left transition-all ${
                        active === i ? 'bg-gradient-to-r from-blue-600 to-violet-600 text-white shadow' : 'hover:bg-gray-50 text-gray-800'
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <span className={`h-2 w-2 rounded-full ${active === i ? 'bg-white' : 'bg-gray-300 group-hover:bg-blue-400'}`} />
                        <span className="text-sm font-semibold">{y.year}</span>
                      </div>
                      <div className={`text-xs ${active === i ? 'text-white/90' : 'text-gray-500'}`}>{y.title}</div>
                    </button>
                  </li>
                ))}
              </ul>
