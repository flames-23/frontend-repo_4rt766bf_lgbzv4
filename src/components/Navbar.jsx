  ...
  return (
    <header className="sticky top-0 z-[9999] relative supports-[backdrop-filter]:bg-white/70 bg-white/90 backdrop-blur border-b border-black/10 shadow-[0_2px_12px_rgba(17,24,39,0.06)]" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-3 items-center h-16 sm:h-[72px] md:h-[88px]">
            ...
            <a href="#home" className="flex items-center gap-2 sm:gap-3 min-w-0">
              <span className="relative inline-flex items-center justify-center shrink-0">
                <span className="p-[2px] rounded-xl" style={{ backgroundImage: 'linear-gradient(90deg, var(--ds-blue), var(--ds-violet))' }}>
                  <span className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-[12px] sm:rounded-[14px] bg-white ring-1 ring-black/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_6px_16px_rgba(17,24,39,0.06)]">
                    <Shield className="h-5 w-5 sm:h-6 sm:w-6" style={{ color: 'var(--ds-blue)' }} />
                  </span>
                </span>
              </span>
              <span className="leading-tight select-none truncate">
                <span className="block text-[13px] sm:text-[14px] font-extrabold tracking-tight text-[color:var(--ds-body-light)]">ALGO UNIVERSITY</span>
                <span className="block text-[11px] sm:text-[12px] subtext">School of CS & AI</span>
              </span>
            </a>
          </div>

          <nav className="hidden md:flex items-center justify-center gap-6 lg:gap-7 whitespace-nowrap">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="group relative inline-flex items-center gap-2 text-[14px] lg:text-[15px] font-semibold tracking-tight transition-colors whitespace-nowrap text-[color:var(--ds-body-light)]"
              >
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-3 pr-1 sm:pr-2 md:pr-3 lg:pr-4">
            <a
              href="#admissions-aseat"
              className="hidden md:inline-flex items-center justify-center rounded-2xl px-5 lg:px-6 py-3.5 text-white text-[14px] lg:text-[15px] font-semibold tracking-tight ring-1 ring-white/10 hover:scale-[1.03] transition-transform"
          ...
          <div className="flex items-center justify-between px-4 h-16 sm:h-20 border-b border-black/10">
            <div className="flex items-center gap-3 min-w-0">
              <span className="p-[2px] rounded-lg" style={{ backgroundImage: 'linear-gradient(90deg, var(--ds-blue), var(--ds-violet))' }}>
                <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-[12px] bg-white ring-1 ring-black/10">
                  <Shield className="h-5 w-5" style={{ color: 'var(--ds-blue)' }} />
                </span>
              </span>
              <div className="leading-tight min-w-0">
                <div className="text-[12px] sm:text-[13px] font-extrabold text-[color:var(--ds-body-light)] truncate">ALGO UNIVERSITY</div>
                <div className="text-[10px] sm:text-[11px] subtext truncate">School of CS & AI</div>
              </div>
            </div>
            <button
              ...
              {navItems.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    onClick={() => setOpen(false)}
                    className="flex items-center justify-between rounded-md px-3 py-3 text-[15px] font-semibold text-[color:var(--ds-body-light)] hover:bg-black/5"
                  >
